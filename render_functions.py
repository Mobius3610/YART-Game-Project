...
### Handles rendering elements of the UI
...
def render_bar(console: Console, current_value: int, maximum_value: int, total_width: int) -> None:
	...
	console.print(x=1, y=45, string=f"{current_value} / {maximum_value}", fg=color.bar_text)

def render_dungeon_level(
	console: Console, dungeon_level: int, location: Tuple[int,int]
) -> None: 
	# Render current level at the player's current location
	x,y = location
